...
Class quiz_model extends CI_Model
{
 ...
 function quiz_list($limit,$gid='')
 {
   ...
   $search=$this->input->post('search');
	...
	if($gid!=""){
		//print_r("1");
  $query = $this -> db -> query("select quiz.*,question_subject.subject_name from quiz join quiz_group on quiz.quid=quiz_group.quid join question_subject on quiz.cid=question_subject.cid where quiz_group.gid='$gid' and $search_type like '%$search%' order by quid asc limit $limit, $nor");
	}else{
		//print_r("2");
	$query = $this -> db -> query("select quiz.*,question_subject.subject_name from quiz join question_subject on quiz.cid=question_subject.cid like '%$search%' 	order by quid desc limit $limit, $nor");
	}
 ...
 if($gid!=""){
  $query = $this -> db -> query("select quiz.*,question_subject.subject_name from quiz join quiz_group on quiz.quid=quiz_group.quid join question_subject on quiz.cid=question_subject.cid where quiz_group.gid='$gid' order by quid asc limit $limit, $nor");
	}else{
	
	$query = $this -> db -> query("select quiz.*,question_subject.subject_name from quiz join question_subject on quiz.cid=question_subject.cid order by quid asc limit $limit, $nor");
...
$this->db->insert('essay_qsn', $insert_data);
 ...
 $institute_id = $this->session->userdata('institute_id');
   ...
   $query = $this -> db -> query("select quiz_group.* from quiz_group where quid='$id' ");
...
//$institute_id = $this->session->userdata('institute_id');
 	...
 	if($qids!=""){
 	$qrr="SELECT qbank.*,question_subject.*,difficult_level.level_name FROM qbank JOIN question_subject ON qbank.cid = question_subject.cid JOIN difficult_level on qbank.did=difficult_level.did where  qid in ($qids) ORDER BY FIELD(qbank.qid, $qids ) ";
...
$kk=$k+1;
 ...
 //$institute_id = $this->session->userdata('institute_id'); 
	...
	$qids=$row['qids'];
//print_r($qids);
	$query = $this -> db -> query("SELECT * FROM  `qbank` JOIN question_subject ON qbank.cid = question_subject.cid WHERE qbank.qid IN ( $qids ) ORDER BY FIELD(qid, $qids )");
 ...
 // check for new test attempt
	...
	$userdata=$this->session->userdata('logged_in');
 	$uid=$userdata['id'];
 	$gid=$userdata['gid'];
	$query = $this -> db -> query("select users.* from users where id='$uid'");
	$row2=$query->row_array();
	// if quiz assign to user group
	$query = $this -> db -> query("select quiz_group.* from quiz_group where gid='$gid' and quid='$quid' ");
	...
	$qids_range=array();
$rng=array();
		$query = $this -> db -> query("SELECT qbank.*,question_subject.* FROM qbank JOIN question_subject ON qbank.cid = question_subject.cid where qid in ($assignqids) ORDER BY FIELD(qbank.qid, $assignqids ) ");
	$qidarr=$query->result_array();
	
	foreach($qidarr as $key => $qid){
	$qids[]=$qid['qid'];
	$subject_names[]=$qid['subject_name'];
	
	
	}
$rngarr=array();
$cateselected=array();
foreach($subject_names as $key => $cval){
...
$qids_range=$rng;
	...
	$query = $this -> db -> query("SELECT qbank.*,question_subject.* FROM qbank JOIN question_subject ON qbank.cid = question_subject.cid where qbank.cid='$cid' and qbank.did='$did' ORDER BY RAND() limit $noq ");
	$qidarr=$query->result_array();
	...
	shuffle($qids); 
	//print_r($qids);
	//$qids_range="0-".sizeof($qids);	
	$qids=implode(",",$qids);
	$roids=implode(",",$roids);
	$cat_names=$subject_names[0];
