...
Class qbank_model extends CI_Model
{
    function question_list($limit, $cid)
    {
        ...
        if ($this->input->post('search_type')) {
            ...
            /*$query = $this -> db -> query("select qbank.*, question_subject.subject_name, difficult_level.level_name from qbank, question_subject, difficult_level where qbank.cid=question_subject.cid and qbank.did=difficult_level.did and $search_type like '$search' and qbank.institute_id = '$institute_id' $extrap order by qid asc limit $limit, $nor");*/

            print_r($search);
            $query = $this->db->query("select qbank.*, question_subject.subject_name, difficult_level.level_name from qbank, question_subject, difficult_level where qbank.cid=question_subject.cid and qbank.did=difficult_level.did and $search_type like '%$search%' $extrap order by qid asc limit $limit, $nor");
        } else {

            $query = $this->db->query("select qbank.*, question_subject.subject_name, difficult_level.level_name from qbank, question_subject, difficult_level where qbank.cid=question_subject.cid and qbank.did=difficult_level.did $extrap order by qid asc limit $limit, $nor");
        }
        if ($query->num_rows() >= 1) {
            return $query->result();
        } else {
...
//echo $ques_type; 
    ...
    {
        //$institute_id = $this->session->userdata('institute_id');
        $query = $this->db->query("SELECT * FROM  `qbank` JOIN question_subject ON qbank.cid = question_subject.cid WHERE qbank.qid='$id'");
    ...
    {
        //$institute_id = $this->session->userdata('institute_id');
        if ($this->db->delete('qbank', array('qid' => $id))) {
            $this->db->delete('q_options', array('qid' => $id));
            return true;
        } else {
            return false;
        }
