...
Class result_model extends CI_Model
{
 ...
 // Number of rows per page
 	...
 	if($user_id!=""){
 	$condi="and users.id='".$user_id."' and publish='1'"; //edit here
 	}else{
 	$condi="";  
 	}
 // to search
...
//  $query = $this -> db -> query("SELECT * FROM  `quiz_result` JOIN users ON quiz_result.uid = users.id JOIN quiz ON quiz_result.quid = quiz.quid where $search_type like '%$search%' $condi and quiz_result.institute_id = '$institute_id' order by quiz_result.rid desc limit $limit, $nor");
   ...
   $query = $this -> db -> query("SELECT * FROM  `quiz_result` JOIN users ON quiz_result.uid = users.id JOIN quiz ON quiz_result.quid = quiz.quid where $search_type like '%$search%' $condi order by quiz_result.rid desc limit $limit, $nor");
   ...
   $query = $this -> db -> query("SELECT * FROM  `quiz_result` JOIN users ON quiz_result.uid = users.id JOIN quiz ON quiz_result.quid = quiz.quid $condi order by quiz_result.rid desc limit $limit, $nor");
...
$res=array();
 ...
 function publish_result($rid){

  if($this->db->query("update quiz_result set publish='1' where rid='$rid' "))
   {
     return true;
   }
   else
...
$query = $this -> db -> query("SELECT * FROM  q_options WHERE oid='$oi' ");
  ...
  $institute_id =$this->session->userdata('institute_id');
	...
	//$query = $this -> db -> query("SELECT * FROM  `qbank` JOIN question_subject ON qbank.cid = question_subject.cid WHERE qbank.qid IN ( $qids ) and qbank.institute_id = '$institute_id'  ORDER BY FIELD(qbank.qid, $qids ) ");
	//$questions=$query->result_array(); 
	
	$query = $this -> db -> query("SELECT * FROM  `qbank` JOIN question_subject ON qbank.cid = question_subject.cid WHERE qbank.qid IN ( $qids ) ORDER BY FIELD(qbank.qid, $qids ) ");
	$questions=$query->result_array();
 ...
 $this->db->join('users','users.id = quiz_result.uid');
 	...
 	$this->db->limit(10);
 		$this->db->where('quiz_result.publish','1');
 	if($user_id !="0"){
