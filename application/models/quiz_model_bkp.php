...
$kk=$k+1;
 ...
 $institute_id = $this->session->userdata('institute_id');
	...
	$query = $this -> db -> query("SELECT * FROM  `qbank` JOIN question_subject ON qbank.cid = question_subject.cid WHERE qbank.qid IN ( $qids ) and qbank.institute_id ='$institute_id'  ORDER BY FIELD(qid, $qids )");
 ...
 // check for new test attempt
	...
	$qids_range=array();
$rng=array();
		$query = $this -> db -> query("SELECT qbank.*,question_subject.* FROM qbank JOIN question_subject ON qbank.cid = question_subject.cid where  qbank.institute_id = '$institute_id' and qid in ($assignqids) ORDER BY FIELD(qbank.qid, $assignqids ) ");
...
$qids_range=$rng;
	...
	$query = $this -> db -> query("SELECT qbank.*,question_subject.* FROM qbank JOIN question_subject ON qbank.cid = question_subject.cid where qbank.cid='$cid' and qbank.did='$did' and qbank.institute_id = '$institute_id' ORDER BY RAND() limit $noq ");
	$qidarr=$query->result_array();
