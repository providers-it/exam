...
$user_id=$logged_in['id'];
 ...
 $data['percentile'] = $this->result_model->get_percentile($quid, $data['result']->uid, $data['result']->score);
   ...
   $t_subject_name=explode(",",$data['result']->subject_name );
   $cct=array();
   $cct_per=array();
   $cct_per_total=array();
   $oids=explode(",",$data['result']->oids);
    foreach(explode(",",$data['result']->qids_range) as $rkey => $rval){
	if(!isset($cct[$t_subject_name[$rkey]])){
	$cct[$t_subject_name[$rkey]]=0;
	}
	$jj=explode("-",$rval);
	$j=$jj[0];
	$k=$jj[1];
		for($i=$j; $i<=$k; $i++){
		   foreach(explode(",",$data['result']->time_spent_ind) as $ckey => $cval){
				if($ckey==$i){
					$cct[$t_subject_name[$rkey]] +=$cval;
					//echo $correct_incorrect[$ckey]."<br>";
					if($correct_incorrect[$ckey] >= 0.1 ){
					$cct_per[$t_subject_name[$rkey]]+=$correct_incorrect[$ckey];
					}else if($oids[$ckey] == "0"){
					$cct_per[$t_subject_name[$rkey]]+=0;
					}else{
					$cct_per[$t_subject_name[$rkey]]+=$correct_incorrect[$ckey];
					}
					
					if(isset($correct_score[$ckey])){
					$cct_per_total[$t_subject_name[$rkey]]+=$correct_score[$ckey];
					}else{
					$cct_per_total[$t_subject_name[$rkey]]+=$correct_score['0'];
					}
...
function publish_result($rid){
	$logged_in=$this->session->userdata('logged_in');
	if($logged_in['su']=="1" && $logged_in['id']=="1"){
	$this->result_model->publish_result($rid);
	redirect('result');
	}else{
