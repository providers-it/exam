    ...
    <div class="searchbox form-group" id="searchbox">
        <form method="post" action="<?php echo site_url('user_data'); ?>">
            <select name="search_type" class="form-control" style="width:150px;float:left;">
                ...
                <option value="users.institute_id">Institute ID</option>
                <option value="institute_data.organization_name">Institute Name</option>
            </select>
                        ...
                        if ($result == false) {
                            ...
                            foreach ($result as $row) {
                                ...
                                <tr>
                                    ...
                                    <td data-th="Action"><a
                                                href="javascript: if(confirm('Do you really want to remove this user?')){ window.location='<?php echo site_url('user_data/remove_user/' . $row->id); ?>'; }"
                                                class="btn btn-danger btn-xs">Remove</a>
                                        <a href="<?php echo site_url('user_data/edit_user/' . $row->id); ?>"
                                           class="btn btn-info btn-xs">Edit</a>
                                        <a href="<?php echo site_url('user_data/login_user_by_admin/' . $row->id); ?>"
                                           class="btn btn-warning btn-xs">Login</a></td>
                                </tr>
                                <?php
                            }
                        }
